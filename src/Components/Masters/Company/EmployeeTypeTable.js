...
export default function EmployeeTypeTable() {
  ...
  const fetchEmployeeTypeData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/MstEmpType/getAllMstEmpTypeDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching employeetype data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (empTypeId) => {
  
    navigate('/employeetype', { state: { empTypeId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">EmployeeType Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.empTypeId}
                      onDoubleClick={() => handleRowDoubleClick(row.empTypeId)}
