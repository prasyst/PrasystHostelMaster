...
export default function DesignationTable() {
  ...
  const fetchDesignationData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/MstDesi/getAllMstDesiDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      }, AuthHeader());
      ...
      if (response.data.Status === 0) {
        ...
        console.error('Error fetching Designation data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (desiId) => {
  
    navigate('/designation', { state: { desiId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">Designation Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.desiId}
                      onDoubleClick={() => handleRowDoubleClick(row.desiId)}
