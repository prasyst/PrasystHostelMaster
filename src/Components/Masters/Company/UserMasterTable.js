...
export default function UserMasterTable() {
  ...
  const fetchUserMstData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/UserMst/getAllUserMstDashBoard', {
        start: 0,
        PageSize: 0,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        const formattedData = response.data.data.map(userMst => ({
          ...userMst,
          userName: userMst.userName || '',
          pwd: userMst.pwd || '',
          mobileNo: userMst.mobileNo || '',
          emailId: userMst.emailId || '',
          remark: userMst.remark || ''
        }));
        setRows(formattedData);
        console.log('data', response);
      } else {
        console.error('Error fetching UserMaster data:', response.data.message);
      }
    } catch (error) {
      console.error('Error fetching UserMaster data:', error);
    }
  };
