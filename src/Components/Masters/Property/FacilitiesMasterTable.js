...
export default function FacilitiesMasterTable() {
  ...
  useEffect(() => {
    fetchFacilitiesMasterData();
  }, []);

  const fetchFacilitiesMasterData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/FacilitiesMst/getAllFacilitiesMstDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching Facilities Master data:', response.data.message);
      }
    } catch (error) {
      console.error('Error fetching Facilities Master data:', error);
    }
  };
  ...
  const handleRowDoubleClick = (facId) => {
  
    navigate('/facilitiesMaster', { state: { facId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">Facilities Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.facId}
                      onDoubleClick={() => handleRowDoubleClick(row.facId)}
