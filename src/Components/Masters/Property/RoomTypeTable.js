...
const columns = [
  { id: 'roomTypeName', label: 'RoomType Name', minWidth: 100 },
...
export default function RoomTypeTable() {
  ...
  const fetchRoomTypeData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/RoomTypeMst/getAllRoomTypeMstDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching roomType data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (roomTypeId) => {
  
    navigate('/roomType', { state: { roomTypeId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">RoomType Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.roomTypeId}
                      onDoubleClick={() => handleRowDoubleClick(row.roomTypeId)}
