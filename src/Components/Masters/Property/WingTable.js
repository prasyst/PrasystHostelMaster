...
const columns = [
  { id: 'wingName', label: 'Wing Name', minWidth: 170 },
...
export default function WingTable() {
  ...
  const fetchWingData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/WingMst/getAllWingMstDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching wing data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (wingId) => {
  
    navigate('/wing', { state: { wingId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">Wing Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.wingId}
                      onDoubleClick={() => handleRowDoubleClick(row.wingId)}
