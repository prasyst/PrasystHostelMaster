...
export default function PropertyTypeTable() {
  ...
  const fetchPropertyTypeData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/PropTypeMst/getAllPropTypeMstDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching propertyType data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (propTypeId) => {
  
    navigate('/propertyType-master', { state: { propTypeId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">PropertyType Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.propTypeId}
                      onDoubleClick={() => handleRowDoubleClick(row.propTypeId)}
