...
const columns = [
  { id: 'roomBedTypeName', label: 'RoomBedType Name', minWidth: 100 },
  { id: 'remark', label: 'Remark', minWidth: 100 }
];

export default function RoomBedTypeTable() {
  ...
  useEffect(() => {
    fetchRoomBedTypeData();
  }, []);

  const fetchRoomBedTypeData = async () => {
  
    try {
      const response = await axios.post(`http://43.230.196.21/api/RoomBedTypeMst/getAllRoomBedTypeMstDashBoard`, {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching roomBedType data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (roomBedTypeId) => {
  
    navigate('/roomBedType', { state: { roomBedTypeId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">RoomBedType Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.roomBedTypeId}
                      onDoubleClick={() => handleRowDoubleClick(row.roomBedTypeId)}
