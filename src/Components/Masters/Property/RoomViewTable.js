...
const columns = [
  { id: 'roomViewNAme', label: 'RoomView Name', minWidth: 100 },
...
export default function RoomViewTable() {
  ...
  const fetchRoomViewData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/RoomView/getAllRoomViewDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching roomView data:', response.data.message);
      }
    } catch (error) {
      console.error('Error fetching roomView data:', error);
    }
  };
  ...
  const handleRowDoubleClick = (roomViewId) => {
  
    navigate('/roomView', { state: { roomViewId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">RoomView Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.roomViewId}
                      onDoubleClick={() => handleRowDoubleClick(row.roomViewId)}
