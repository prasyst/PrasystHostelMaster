...
export default function CityTable() {
  ...
  const fetchCityData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/CityMst/getAllcityMstDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      }, AuthHeader());
      ...
      if (response.data.Status === 0) {
        ...
        console.error('Error fetching city data:', response.data.message);
      }
    } catch (error) {
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">City Master</Typography>
          </Breadcrumbs>
