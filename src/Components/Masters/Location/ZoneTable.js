...
export default function ZoneTable() {
  ...
  const fetchZoneData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/zoneRegionMst/getAllzoneRegionMstDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching zone data:', response.data.message);
      }
    } catch (error) {
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">Zone Master</Typography>
          </Breadcrumbs>
