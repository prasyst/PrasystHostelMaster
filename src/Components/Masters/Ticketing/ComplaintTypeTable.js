...
const columns = [
  { id: 'compTypeName', label: 'Type', minWidth: 100 },
...
export default function ComplaintTypeTable() {
  ...
  const fetchComplaintTypeData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/ComplaintTypeMst/getAllComplaintTypeMstDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        const formattedData = response.data.data.map(complaintType => ({
          ...complaintType,
          compTypeName: complaintType.compTypeName || '',
          compTypeAbrv: complaintType.compTypeAbrv,
          compTypeRemark: complaintType.compTypeRemark
        }));
        setRows(formattedData);
        console.log('data', response);
      } else {
        console.error('Error fetching complaintType master data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (compTypeId) => {
  
    navigate('/complaintType', { state: { compTypeId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">ComplaintType Master</Typography>
          </Breadcrumbs>
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.compTypeId}
                      onDoubleClick={() => handleRowDoubleClick(row.compTypeId)}
