...
const columns = [
  { id: 'sourceName', label: 'Source Name', minWidth: 100 },
  { id: 'remark', label: 'Remark', minWidth: 100 }
];

export default function SourceMstTable() {
  ...
  const fetchSourceMstData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/MstSource/getAllMstSourceDashBoard', {
        start: 0,
        PageSize: 0,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        const formattedData = response.data.data.map(source => ({
          ...source,
        ...
        console.error('Error fetching source master data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (sourceId) => {
  
    navigate('/sourceMaster', { state: { sourceId ,mode: 'view'}} );
  };
      ...
      <Box sx={{ maxWidth: '100vw', overflowX: 'hidden' }}>
        ...
        >
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Typography color="text.primary">Source Master</Typography>
          </Breadcrumbs>
                ...
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      ...
                      onDoubleClick={() => handleRowDoubleClick(row.sourceId)}
