...
export default function InstituteMasterTable() {
  ...
  useEffect(() => {
    fetchInstituteMstData();
  }, []);

  const fetchInstituteMstData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/MstInstitute/getAllMstInstituteDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        const formattedData = response.data.data.map(institute => ({
          ...institute,
          instituteName: institute.instituteName || '',
          addr: institute.addr,
          remark: institute.remark
        }));
        setRows(formattedData);
        console.log('data', response);
      } else {
        console.error('Error fetching institute master data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (instituteId) => {
            ...
            <TableHead>
              ...
              <TableBody>
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={row.instituteId}
                      onDoubleClick={() => handleRowDoubleClick(row.instituteId)}
