...
export default function CourseMasterTable() {
  ...
  const fetchCourseMstData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/MstCourse/getAllMstCourseDashBoard', {
        start: 0,
        PageSize: 0,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        ...
        console.error('Error fetching course master data:', response.data.message);
      }
    } catch (error) {
                ...
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      ...
                      onDoubleClick={() => handleRowDoubleClick(row.courseId)}
