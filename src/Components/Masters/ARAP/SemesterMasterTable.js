...
export default function SemesterMasterTable() {
  ...
  useEffect(() => {
    fetchSemesterMstData();
  }, []);

  const fetchSemesterMstData = async () => {
    try {
      const response = await axios.post('http://43.230.196.21/api/MstSemester/getAllMstSemesterDashBoard', {
        start: 1,
        PageSize: 1,
        SearchText: ""
      });
      ...
      if (response.data.status === 0) {
        const formattedData = response.data.data.map(semester => ({
          ...semester,
        ...
        console.error('Error fetching semester master data:', response.data.message);
      }
    } catch (error) {
  ...
  const handleRowDoubleClick = (semesterId) => {
                ...
                {filteredRows
                  ...
                  .map((row) => {
                    return (
                      ...
                      onDoubleClick={() => handleRowDoubleClick(row.semesterId)}
